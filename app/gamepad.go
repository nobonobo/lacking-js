...
func (g *Gamepad) refresh() {
	...
	if g.isConnected {
		g.isSupported = true
		g.isSteering = jsGamepad.Get("mapping").String() != "standard"
		g.deadzoneStick = 0.001
	} else {
		g.isSupported = false
	}
	if g.isSupported {
		...
		if !g.isSteering {
			...
			g.leftStickY = 0.0  // axes.Index(1).Float()
			g.rightStickX = 0.0 // axes.Index(2).Float() side-brake
			g.rightStickY = 0.0 // axes.Index(3).Float() clutch
			brake := (axes.Index(1).Float() + 0.8) / (0.85 * 2)
